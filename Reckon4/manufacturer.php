    ...
    <div class="container">
        ...
        <?php 
                    ...
                    if (isset($_GET['Quantity'])){
                        $result = mysqli_query($clink, "SELECT * FROM `advertisments` ORDER BY `advertisments`.`Quantity` ASC  ");
                    ...
                    $result = mysqli_query($clink, "SELECT * FROM `advertisments` ORDER BY `advertisments`.`AdsID` DESC     ");
                        ...
                        $result = mysqli_query($clink, "SELECT * FROM `advertisments` ORDER BY `advertisments`.`Address` ASC   ");
                    ...
                    if(mysqli_num_rows($result)>0){
						...
						while($row = mysqli_fetch_assoc($result)){
                            ...
                            <span class='float-left'> $ {$row['Quantity']}</span>
                                <div class='card-body'>
                                <h5 class='card-title'>{$row['Stage']}</h5>
